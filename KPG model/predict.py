...
tf.app.flags.DEFINE_string('model_dir', "model/dir", 'Path to load model checkpoints')
...
class Seq2SeqPredictor:
    ...
    def predict(self, keywords):
        ...
        for keyword in keywords:
            content_inputs, content_inputs_length,keyword_inputs,keyword_length,keyword_teams,keyword_teams_length= prepare_batch_predict_data(keyword,keywords,x,
                                                            previous=sentences,
                                                            prev=FLAGS.prev_data,
                                                            rev=FLAGS.rev_data,
                                                            align=FLAGS.align_data)

            predicted_batch = self.model.predict(self.sess,content_inputs,content_inputs_length,keyword_inputs,keyword_length,keyword_teams,keyword_teams_length)
