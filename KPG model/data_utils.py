...
def prepare_batch_predict_data(keyword, keywords,n,previous, prev=True, rev=False, align=False):
    # previous sentences
    content_tmp = [SEP_TOKEN]
    keyword_teams = []
    keyword_teams_length = []
    keyword_tmp=[]
    ...
    for keyword_ in keywords:
        keywords_ints_1 = process_sentence(keyword_, rev=True, pad_len=None)
        keyword_tmp.append(keywords_ints_1)
    if n == 0:
        keyword_teams = keyword_tmp[0] + keyword_tmp[1] + keyword_tmp[2] + keyword_tmp[3]
    if n == 1:
        keyword_teams = keyword_tmp[1] + keyword_tmp[0] + \
                       keyword_tmp[2] + keyword_tmp[3]
    if n == 2:
        keyword_teams = keyword_tmp[2] + keyword_tmp[3] + \
                       keyword_tmp[0] + keyword_tmp[1]
    if n == 3:
        keyword_teams = keyword_tmp[3] + keyword_tmp[2] + \
                       keyword_tmp[0] + keyword_tmp[1]
    ...
    keyword_teams_length = np.array([len(keyword_teams)])
    keyword_teams = fill_np_matrix([keyword_teams], 1, PAD_TOKEN)
    return content_inputs, content_inputs_length,keyword_inputs,keyword_length,keyword_teams,keyword_teams_length
        ...
        while not stop:
            ...
            for i in range(batch_size):
                ...
                if not line:
                    ...
                    keywords_ints_1 = process_sentence(keywords, rev=True, pad_len=None)
                    keyword_tmp.append(keywords_ints_1)
                    ...
                    content_tmp += [SEP_TOKEN] + sentence_ints
                    if x in [3, 7, 11, 15, 19, 23, 27, 31, 35, 39, 43, 47, 51, 55, 59, 63, 67, 71, 75, 79, 83, 87, 91,
                             95, 99, 103, 107, 111, 115, 119, 123, 127]:
                        for n in range(4):
                            keyword_team=[]
                            if n == 0:
                                keyword_team = keyword_tmp[0] + keyword_tmp[1] + keyword_tmp[2] + keyword_tmp[3]
                            if n == 1:
                                keyword_team = keyword_tmp[1] + keyword_tmp[0] + \
                                               keyword_tmp[2] + keyword_tmp[3]
                            if n == 2:
                                keyword_team = keyword_tmp[2] + keyword_tmp[3] + \
                                               keyword_tmp[0] + keyword_tmp[1]
                            if n == 3:
                                keyword_team = keyword_tmp[3] + keyword_tmp[2] + \
                                               keyword_tmp[0] + keyword_tmp[1]

                            keyword_teams.append(keyword_team)
                            keyword_teams_length.append(len(keyword_team))
                            # print("keyword_teams", keyword_teams)
                            # print(keyword_teams_length)
                x  +=1
            if len(keyword) == batch_size:
                ...
                keyword_teams_fill=fill_np_matrix(keyword_teams,batch_size,PAD_TOKEN)
                keyword_lengths = np.array(keyword_length)
                content_lengths = np.array(content_length)
                target_lengths=np.array(target_length)
                keyword_teams_length_fill=np.array(keyword_teams_length)
                yield keywordes, keyword_lengths,contents,content_lengths,targets,target_lengths,keyword_teams_fill,keyword_teams_length_fill
