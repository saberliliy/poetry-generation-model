...
class Seq2SeqModel:
    ...
    def build_model(self):
        ...
        self.keyword_teams_encoder()
        self.bulid_content_encoder()
        self.build_keyword_encoder()
        self.build_decoder()
    ...
    def init_placeholders(self):
        ...
        self.keyword_inputs = tf.placeholder(
            name='keyword_inputs',
            shape=(None, None),
            dtype=tf.int32
        )
        self.keyword_teams=tf.placeholder(
            name='keyword_teams',
            shape=(None, None),
            dtype=tf.int32
        )
        self.keyword_teams_lengths = tf.placeholder(
            name='keyword_teams_lengths',
            shape=(None,),
            dtype=tf.int32
        )
        ...
        self.content_inputs=tf.placeholder(
            name='content_inputs',
            shape=(None,None),
            dtype=tf.int32
        )
        self.content_inputs_length=tf.placeholder(
            name="content_inputs_length",
            shape=(None,),
            dtype=tf.int32
        )
        # use dynamic batch_size based on input
        self.batch_size = tf.shape(self.keyword_inputs)[0]
    ...
    def keyword_teams_encoder(self):
        print('Building keyword_teams_encoder...')
        with tf.variable_scope('keyword_teams'):
            self.keyword_teams_inputs_embedded = tf.nn.embedding_lookup(
                params=self.embedding,
                ids=self.keyword_teams
            )
            self.keyword_teams_cell_fw=self.build_rnn_depth_cell_half()
            self.keyword_teams_cell_bw=self.build_rnn_depth_cell_half()
            self.keyword_teams_outputs_fw_bw, self.keyword_teams_last_state_fw_bw = bidirectional_dynamic_rnn(
                cell_fw=self.keyword_teams_cell_fw,
                cell_bw=self.keyword_teams_cell_bw,
                inputs=self.keyword_teams_inputs_embedded,
                sequence_length=self.keyword_teams_lengths,
                dtype=self.dtype,
                time_major=False,
            )
            ...
            self.keyword_teams_fw, self.keyword_teams_bw = self.keyword_teams_last_state_fw_bw

            self.keyword_teams_fw, self.keyword_teams_bw = self.keyword_teams_last_state_fw_bw
            encoder_last_state_zipped = zip(self.keyword_teams_fw, self.keyword_teams_bw)
            self.ketword_teams_c=[]
            self.ketword_teams_h=[]
            for fw, bw in encoder_last_state_zipped:
                self.ketword_teams_c.append(tf.concat([fw.c,bw.c],1))
                self.ketword_teams_h.append(tf.concat([fw.h, bw.h], 1))
    def bulid_content_encoder(self):
        print('Building content_encoder...')
        with tf.variable_scope('pre'):
            self.conent_inputs_embedded = tf.nn.embedding_lookup(
                params=self.embedding,
                ids=self.content_inputs
            )
            self.pre_cell_fw=self.build_rnn_depth_cell()
            self.pre_cell_bw=self.build_rnn_depth_cell()
            self.pre_outputs_fw_bw, self.pre_last_state_fw_bw = bidirectional_dynamic_rnn(
                cell_fw=self.pre_cell_fw,
                cell_bw=self.pre_cell_bw,
                inputs=self.conent_inputs_embedded,
                sequence_length=self.content_inputs_length,
                dtype=self.dtype,
                time_major=False,
            )
            self.pre_outputs_fw, self.pre_outputs_bw = self.pre_outputs_fw_bw
    ...
    def build_keyword_encoder(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            if self.bidirectional:
                # Build encoder cell
                self.encoder_cell_fw = self.build_rnn_depth_cell_half()
                self.encoder_cell_bw = self.build_rnn_depth_cell_half()
                # Encode input sequences into context vectors
                # encoder_outputs: [batch_size, time_step, cell_output_size]
                # encoder_last_state: [batch_size, cell_output_size]
                self.encoder_outputs_fw_bw, self.encoder_last_state_fw_bw = bidirectional_dynamic_rnn(
                ...
                self.ketword_h = []
                for fw, bw in encoder_last_state_zipped:
                    self.ketword_c.append(tf.concat([fw.c, bw.c], 1))
                    self.ketword_h.append(tf.concat([fw.h, bw.h], 1))
    ...
    def build_decoder(self):
        ...
        with tf.variable_scope('decoder'):
            ...
            encoder_last_state_zipped = zip(self.ketword_c, self.ketword_h,self.ketword_teams_c,self.ketword_teams_c)
            keyword_last_state_list = [
                rnn.LSTMStateTuple(c=tf.concat([kc, tc], 1), h=tf.concat([kh, th], 1))
                for kc, kh,tc,th in encoder_last_state_zipped]
            self.keyword_encoder_last_state = tuple(keyword_last_state_list)
    ...
    def build_rnn_depth_cell(self):
        self.encoder_cell_list=[self.build_single_cell(self.hidden_units) for _ in range(self.depth)]

        return rnn.MultiRNNCell(self.encoder_cell_list)

    def build_rnn_depth_cell_half(self):
        self.encoder_cell_list = [self.build_single_cell(self.hidden_units/2) for _ in range(self.depth)]
    ...
    def build_decoder_cell(self):
        ...
        self.attention_mechanism = seq2seq.BahdanauAttention(
            ...
            memory=self.content_outputs,
            memory_sequence_length=self.content_inputs_length
        )

        self.decoder_cell_list = [self.build_single_cell(self.decoder_hidden_units) for _ in range(self.depth)]
        ...
        self.decoder_cell_list[-1] = seq2seq.AttentionWrapper(
            ...
            initial_cell_state=self.keyword_encoder_last_state[-1],
        ...
        # embed()
        initial_state = [state for state in self.keyword_encoder_last_state]
        initial_state[-1] = self.decoder_cell_list[-1].zero_state(
            batch_size=self.batch_size,
            dtype=self.dtype
        )
        decoder_initial_state = tuple(initial_state)  #将列表转换为元组
    ...
    def train(self, sess, content_inputs, content_inputs_length,keyword_inputs,keyword_length,
              decoder_inputs, decoder_inputs_length,keyword_teams,keyword_teams_lengths):
        ...
        input_feed = self.check_feeds(content_inputs, content_inputs_length,keyword_inputs,keyword_length,
                                      decoder_inputs, decoder_inputs_length,keyword_teams,keyword_teams_lengths, False)
    ...
    def predict(self, sess, content_inputs, content_inputs_length,keyword_inputs,keyword_length,keyword_teams,keyword_teams_lengths):
        input_feed = self.check_feeds(content_inputs, content_inputs_length,keyword_inputs,keyword_length,
                                      decoder_inputs=None, decoder_inputs_length=None,keyword_teams=keyword_teams,keyword_teams_lengths=keyword_teams_lengths,
                                      predict=True)
    ...
    def check_feeds(self, content_inputs, content_inputs_length,keyword_inputs,keyword_length,
                    decoder_inputs, decoder_inputs_length, keyword_teams,keyword_teams_lengths,predict):
        """
        Args:
          encoder_inputs: a numpy int matrix of [batch_size, max_source_time_steps]
              to feed as encoder inputs
        ...
        input_feed = {}
        input_feed[self.content_inputs.name]=content_inputs
        input_feed[self.content_inputs_length.name]=content_inputs_length
        input_feed[self.keyword_inputs.name] = keyword_inputs
        input_feed[self.keyword_length.name] = keyword_length
        input_feed[self.keyword_teams.name]=keyword_teams
        input_feed[self.keyword_teams_lengths.name]=keyword_teams_lengths
        if not predict:
