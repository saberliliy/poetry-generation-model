...
class Seq2SeqModel:
    ...
    def init_placeholders(self):
        ...
        self.pre_inputs=tf.placeholder(
            name='pre_inputs',
            shape=(None,None),
            dtype=tf.int32
        )
        self.pre_inputs_lens=tf.placeholder(
            name="pre_inputs_lens",
            shape=(None,),
            dtype=tf.int32
        )
    ...
    def bulid_pre_RNN(self):
        print('Building encoder...')
        with tf.variable_scope('pre'):
            self.pre_inputs_embedded = tf.nn.embedding_lookup(
                params=self.embedding,
                ids=self.encoder_inputs
            )
            # self.pre_cell = self.build_pre_cell()
            # self.pre_outputs, self.pre_last_state = tf.nn.dynamic_rnn(
            #     cell=self.pre_cell,
            #     inputs=self.pre_inputs_embedded,
            #     sequence_length=self.pre_inputs_lens,
            #     dtype=self.dtype,
            #     time_major=False
            # )
            self.pre_cell_fw=self.build_pre_cell()
            self.pre_cell_bw=self.build_pre_cell()
            self.pre_outputs_fw_bw, self.pre_last_state_fw_bw = tf.nn.bidirectional_dynamic_rnn(
            ...
            self.pre_outputs = tf.concat([self.pre_outputs_fw, self.pre_outputs_bw], 2)

            self.pre_last_state_fw, self.pre_last_state_bw = self.pre_last_state_fw_bw
            pre_last_state_zipped = zip(self.pre_last_state_fw, self.pre_last_state_bw)
            pre_last_state_list = [rnn.LSTMStateTuple(c=tf.concat([fw.c, bw.c], 1), h=tf.concat([fw.h, bw.h], 1))
                                       for fw, bw in pre_last_state_zipped]
            self.pre_last_state = tuple(pre_last_state_list)
    def build_single_cell(self, hidden_units):
        ...
        return cell
    def build_pre_cell(self):
        self.pre_cell_list = [self.build_single_cell(self.hidden_units) for _ in range(self.depth)]

        return rnn.MultiRNNCell(self.pre_cell_list)
    def build_encoder_cell(self):
        self.encoder_cell_list=[self.build_single_cell(self.hidden_units) for _ in range(self.depth)]
        initial_state_fw = [state for state in self.pre_last_state_fw]
        initial_state_bw=[state for state in self.pre_last_state_fw]
        self.encoder_initial_state = [state for state in self.pre_last_state]
        self.encoder_initial_state[-1] = self.encoder_cell_list[-1].zero_state(
            batch_size=self.batch_size,
            dtype=self.dtype
        )
        self.encoder_initial_state=tuple(self.encoder_initial_state)
        self.encoder_initial_state_fw= tuple(initial_state_fw)
        self.encoder_initial_state_bw=tuple(initial_state_bw)
        ...
        with tf.variable_scope('encoder'):
            ...
            if self.bidirectional:
                ...
                self.encoder_outputs_fw_bw, self.encoder_last_state_fw_bw = tf.nn.bidirectional_dynamic_rnn(
                    ...
                    dtype=self.dtype,
                    time_major=False,
                    initial_state_fw=self.encoder_initial_state_fw,
                    initial_state_bw=self.encoder_initial_state_bw
                )
    ...
    def build_decoder_cell(self):
        ...
        memory=self.pre_outputs
        if self.mode=="predict":
            memory=seq2seq.tile_batch(memory,self.beam_width)
            pre_inputs_lens=seq2seq.tile_batch(self.pre_inputs_lens,self.beam_width)
            encoder_last_state=seq2seq.tile_batch(self.encoder_last_state,self.beam_width)
            bs=self.batch_size*self.beam_width
            pre_last_state=seq2seq.tile_batch(self.pre_last_state,self.beam_width)
        else:
            bs=self.batch_size
            pre_inputs_lens=self.pre_inputs_lens
            encoder_last_state = self.encoder_last_state
            pre_last_state=self.pre_last_state
        self.attention_mechanism = seq2seq.BahdanauAttention(
            num_units=self.decoder_hidden_units,
            memory=memory,
            memory_sequence_length=pre_inputs_lens,
            normalize=True
        )
        ...
        self.decoder_cell_list[-1] = seq2seq.AttentionWrapper(
            ...
            initial_cell_state=pre_last_state[-1],
    ...
    def build_predict_decoder(self):
        ...
        inference_decoder = tf.contrib.seq2seq.BeamSearchDecoder(cell=self.decoder_cell, embedding=lambda inputs: tf.nn.embedding_lookup(self.embedding, inputs),
        ...
        self.decoder_pred_decode = decoder_outputs.predicted_ids
    ...
    def train(self, sess, encoder_inputs, encoder_inputs_length,pre_inputs,pre_inputs_lens,
        ...
        input_feed = self.check_feeds(encoder_inputs, encoder_inputs_length,pre_inputs,pre_inputs_lens,
    ...
    def predict(self, sess, encoder_inputs, encoder_inputs_length,pre_inputs,pre_inputs_lens):
        input_feed = self.check_feeds(encoder_inputs, encoder_inputs_length,pre_inputs,pre_inputs_lens,
    ...
    def check_feeds(self, encoder_inputs, encoder_inputs_length,pre_inputs,pre_inputs_lens,
        ...
        input_feed = {}
        input_feed[self.pre_inputs.name]=pre_inputs
        input_feed[self.pre_inputs_lens.name]=pre_inputs_lens
